---
layout: post
title: 文字コード (Unicode, JIS, SJIS, EUC) について
date: 2023/2/3
---
# はじめに
本記事では Unicode と JIS/SJIS/EUC に関して、エンコーディング方法を中心に説明しています。
Unicode 関連の情報は多いですが、JIS/SJIS/EUC などの文字コードに関する情報は減ってきていますので参考なれば幸いです。
元の記事は 2000 年代に書いたものですがブログの記事数を稼ぐため若干内容を更新して掲載しました。

# 術語の整理

Unicode は世界中の文字を単一の文字集合（文字セット）で取り扱えるようにした文字コードの業界規格です。
Unicode は符号化文字集合の規格でもあり、同時に UTF-8 や UTF-16 と呼ばれる符号化方式（CEF, CES）を定めた規格でもあります。
...
## 文字符号化形式（ＣＥＦ）
 - 文字列を計算機のメモリ上で扱いやすい形式に変換する為の方式(符号化方式)
...
## 文字符号化スキーム（ＣＥＳ）
 - 文字列をファイルに保存したりネットワークに流したりする形式に変換する為の方式(符号化方式)
...
ここで「コードポイント」はあくまで机上で採番した単なる出席番号であって、文字コード（＝計算機上のメモリ表現）とは異なりますので注意してください。

 - 文字コード

一般的に「文字コード」と言う用語は少し曖昧です。符号化文字集合や文字符号化方式をまとめた規則体系を表わす用語であり、
同時に、計算機上でのメモリ表現(つまり計算機のメモリ上で表現されているデータ値)を指して文字コードと呼ぶ場合もあります。
